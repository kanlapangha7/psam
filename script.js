...
// เพิ่มเข้าไปในฟังก์ชัน window.onload หรือ document.addEventListener('DOMContentLoaded', ...)
document.addEventListener('DOMContentLoaded', function() {
    // ค้นหา canvas element
    canvasElement = document.querySelector('.output-canvas');
    if (canvasElement) {
        canvasCtx = canvasElement.getContext('2d');
        console.log('Canvas context initialized successfully');
    } else {
        console.error('ไม่พบ canvas element (.output-canvas) กรุณาตรวจสอบ HTML');
    }
    
    // เรียกฟังก์ชันอื่นๆ ที่ต้องการเริ่มต้นต่อไป
});
